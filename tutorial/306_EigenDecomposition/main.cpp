...
int main(int argc, char* argv[]) {
  ...
  if (!read_triangle_mesh(
	  argc > 1 ? argv[1] : TUTORIAL_SHARED_PATH "/dino.obj", V, F)) {
  ...
  viewer.callback_key_down = [&](igl::viewer::Viewer& viewer, unsigned char key,
				 int) -> bool {
    switch (key) {
      default:
				return false;
      case ' ': {
				...
				VectorXd Z = bbd * 0.5 * U.col(c);
				Eigen::MatrixXd C;
				//igl::parula(U.col(c).eval(), true, C);
				igl::colormap(igl::COLOR_MAP_TYPE_REDBLUE, U.col(c).eval(), true, C);
				//cout << C << "\n";
				cout << "Eigen vector : " << c << "\n";
				c = (c + 1) % U.cols();
				if (twod) {
